...
import { CalendarClock, CloudSun, ListTodo, LogOut, Mail, Send, Plus, Calendar } from "lucide-react";
...
interface CockpitDashboardProps {
  ...
  profile?: Profile;
  onLogout?: () => void;
  onContextChange?: (ctx: { tasks: TaskData[]; emails: any[]; calendar: CalendarEvent[] }) => void;
}
...
export default function CockpitDashboard({ onAction, profile, onLogout, onContextChange }: CockpitDashboardProps) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [weatherRes, tasksRes, gmailRes, calendarRes] = await Promise.allSettled([
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="w-3 h-3 text-white/15 group-hover:text-cyan-300/70">›</span>
                    </div>
